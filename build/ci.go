// Copyright 2016 The striatum Authors
// This file is part of the striatum library.
//
// The striatum library is free software: you can redistribute it and/or modify
// it under the terms of the GNU Lesser General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// The striatum library is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU Lesser General Public License for more details.
//
// You should have received a copy of the GNU Lesser General Public License
// along with the striatum library. If not, see <http://www.gnu.org/licenses/>.
...
var (
	// Files that end up in the striatum*.zip archive.
	gethArchiveFiles = []string{
		"COPYING",
		executablePath("striatum"),
	}

	// Files that end up in the striatum-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert Engram contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
...
func doArchive(cmdline []string) {
	...
	var (
		...
		geth     = "striatum-" + basegeth + ext
		alltools = "striatum-alltools-" + basegeth + ext
	)
...
func doDocker(cmdline []string) {
	...
	//  - ethereum/client-go:latest                            - Pushes to the master branch, Striatum only
	//  - ethereum/client-go:stable                            - Version tag publish on GitHub, Striatum only
	//  - ethereum/client-go:alltools-latest                   - Pushes to the master branch, Striatum & tools
	//  - ethereum/client-go:alltools-stable                   - Version tag publish on GitHub, Striatum & tools
	//  - ethereum/client-go:release-<major>.<minor>           - Version tag publish on GitHub, Striatum only
	//  - ethereum/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, Striatum & tools
	//  - ethereum/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, Striatum only
	//  - ethereum/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, Striatum & tools
	var tags []string
	...
	if len(*manifest) != 0 {
		...
		// Assemble and push the striatum manifest image
		for _, tag := range tags {
...
func doDebianSource(cmdline []string) {
	var (
		...
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "striatum-ci")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = os.MkdirTemp("", "striatum-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "Engram Builds <dev@engramnet.io>"
	}
	return debMetadata{
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "striatum.exe" {
			gethTool = file
		} else {
	...
	installer, err := filepath.Abs("striatum-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	if err != nil {
