...
var (
	...
	NetworkIdFlag = &cli.Uint64Flag{
		Name:     "networkid",
		Usage:    "Explicitly set network id (integer)(For testnets: use --engram, --goerli, --sepolia, --holesky instead)",
	...
	EngramFlag = &cli.BoolFlag{
		Name:     "engram",
		Usage:    "Engram Tokio Network: pre-configured proof-of-stake test network",
		Category: flags.EthCategory,
	}
	// Dev mode
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		if ctx.Bool(EngramFlag.Name) {
			return filepath.Join(path, "engram")
		}
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	case ctx.Bool(EngramFlag.Name):
		urls = params.EngramBootnodes
...
func SetDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.Bool(EngramFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "engram")
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	CheckExclusive(ctx, MainnetFlag, DeveloperFlag, EngramFlag, GoerliFlag, SepoliaFlag, HoleskyFlag)
	...
	case ctx.Bool(EngramFlag.Name):
		...
		cfg.Genesis = core.DefaultEngramGenesisBlock()
		SetDNSDiscoveryDefaults(cfg, params.EngramGenesisHash)
	case ctx.Bool(HoleskyFlag.Name):
		if !ctx.IsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 17000
		}
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	case ctx.Bool(EngramFlag.Name):
		genesis = core.DefaultEngramGenesisBlock()
	case ctx.Bool(SepoliaFlag.Name):
