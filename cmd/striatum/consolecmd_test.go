...
func TestConsoleWelcome(t *testing.T) {
	...
	geth.Expect(`
Welcome to the Striatum JavaScript console!

instance: Striatum/v{{gethver}}/{{goos}}-{{goarch}}/{{gover}}
coinbase: {{.Etherbase}}
at block: 0 ({{niltime}})
 datadir: {{.Datadir}}
 modules: {{apis}}
...
func TestAttachWelcome(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		ipc = `\\.\pipe\striatum` + strconv.Itoa(trulyRandInt(100000, 999999))
	} else {
		ipc = filepath.Join(t.TempDir(), "striatum.ipc")
	}
